...
class LDATopicModel:
    ...
    def initialize_counts(self):
        ...
        for doc_id in range(0, len(self.documents)):
            ...
            for word_id in range(0, len(doc)):
                word = doc[word_id]
    ...
    def theta_d_i(self, doc_id, z):
        ...
        """
        # >>> YOUR ANSWER HERE
        ndi = self.doc_topic_counts[doc_id][z]
        return (ndi + self.alpha)/(self.doc_lengths[doc_id] + 1)
        # >>> END YOUR ANSWER
    ...
    def phi_i_v(self, z, word):
        ...
        """
        # >>> YOUR ANSWER HERE
        viv = self.topic_word_counts[z][word]
        return (viv + self.beta) / (self.topic_counts[z] + (self.beta*(len(self.topic_word_counts[z]))))
        # >>> END YOUR ANSWER
    ...
    def train(self):
        ...
        for iteration in range(self.iterations):
            ...
            for doc_id in range(0, len(self.documents)):
                ...
                for word_id in range(0, len(doc)):
                    word = doc[word_id]
                    z = self.assignments[doc_id, word_id]
                    self.decrement_counts(doc_id, word, z)
                    z_weights = []
                    for topic in self.topics:
                        z_weights.append(self.theta_d_i(doc_id, topic) * self.phi_i_v(topic, word))

                    new_z = random.choices(self.topics, weights=z_weights)[0]
                    self.assignments[doc_id, word_id] = new_z
                    self.increment_counts(doc_id, word, new_z)
