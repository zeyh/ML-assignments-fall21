...
class LDATopicModel:
    def __init__(self, K=3, dataset="animals", iterations=100):
        ...
        if dataset == "animals":
            self.documents = [
                [w.lower() for w in line.split()] for line in open("animals.txt")
            ]
        elif dataset == "20_newsgroups_subset":
            self.documents = []
            for doc in fetch_20newsgroups(
                categories=("sci.space", "misc.forsale", "talk.politics.misc"),
    ...
    def filter_docs(self):
        ...
        for word in dfs:
            ...
            if (
                df_proportion < 0.005
            ):  # remove words appearing in less than 0.5% of documents
                to_remove.add(word)
            elif (
                df_proportion > 0.5
            ):  # remove words appearing in more than 50% of documents
                to_remove.add(word)
            elif all(
                c in string.punctuation + string.digits for c in word
            ):  # remove punctuation and numerical
                to_remove.add(word)
    ...
    def initialize_counts(self):
        ...
        self.doc_topic_counts = {
            doc_id: Counter() for doc_id in range(len(self.documents))
        }
    ...
    def print_topics(self):
        ...
        for z in self.topics:
            ...
            print(
                ...
                " ".join(
                    str(w) + " " + "{:.3f}".format(v)
                    for w, v in sorted(vals.items(), key=lambda x: x[1], reverse=True)[
                        0:10
                    ]
                ),
            )
    ...
    def theta_d_i(self, doc_id, z):
        ...
        """
        # Number of words in the document assigned to each topic
        # -----
        # Length of the document + alpha * num topics
        numerator = self.alpha + self.doc_topic_counts[doc_id][z]
        document_length = self.doc_lengths[doc_id]
        denominator = self.alpha * self.num_topics + document_length
    ...
    def phi_i_v(self, z, word):
        ...
        """
        # number words assigned to this topic with the given word identity + self.beta
        # -------------
        # number of words total assigned to topic + self.beta * vocabulary size
        numerator = self.beta + self.topic_word_counts[z][word]
        denominator = self.beta * len(self.vocabulary) + self.topic_counts[z]
        return numerator / denominator
    ...
    def train(self):
        ...
        for iteration in range(self.iterations):
            ...
            for doc_id, doc in enumerate(self.documents):
                for word_id, word in enumerate(doc):
                    prev_topic = self.assignments[doc_id, word_id]
                    self.decrement_counts(doc_id, word, prev_topic)
                    weights = [
                        self.theta_d_i(doc_id, topic) * self.phi_i_v(topic, word)
                        for topic in self.topics
                    ]
                    new_topic = random.choices(self.topics, weights=weights)[0]
                    # print(new_topic, weights)
                    self.assignments[doc_id, word_id] = new_topic
                    self.increment_counts(doc_id, word, new_topic)
...
if __name__ == "__main__":
    ...
    # print(lda.phi_i_v(0, 'pond'))
    # print(lda.theta_d_i(42, 1))
    lda.train()
